...
from mattermake.utils.vocab import vocab, encode_slice, stoi, itos
...
def prepare_slices_data(
    ...
    data = {"embeddings": data["embeddings"], "slices": data["slices"]}
    ...
    if data_limit_factor < 1.0:
        ...
        log.info(
            f"Limiting data to {data_limit_factor:.1%} of original size: {limit_samples}/{total_samples} samples"
        )
    ...
    train_data = {"embeddings": train_emb, "slice_ids": train_slice_ids}
    val_data = {"embeddings": val_emb, "slice_ids": val_slice_ids}
