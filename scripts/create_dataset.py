...
class DatasetCreator:
    ...
    def process_structure_from_cif_file(self, cif_file: Path) -> Optional[Dict[str, Any]]:
    ...
    def process_structure_from_cif_string(self, cif_string: str, material_id: str = "unknown") -> Optional[Dict[str, Any]]:
    ...
    def process_structure(self, structure, material_id: str = "unknown") -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            composition_vector = get_composition_vector(structure, self.element_vocab_size)
            ...
            for atomic_number, wyckoff_idx, coords in atom_data:
                ...
                if atomic_number <= 0 or atomic_number > self.element_vocab_size:
                    log.warning(f"Invalid atomic number {atomic_number} for {material_id}")
                    continue
                ...
                if self.validate_wyckoff:
                    ...
                    if not wyckoff_valid:
                        if self.filter_invalid_wyckoff:
                            log.warning(f"Filtering out invalid Wyckoff index {wyckoff_idx} for {material_id}")
                            continue
                        else:
                            # Use corrected index if available
                            wyckoff_idx = corrected_idx if corrected_idx is not None else 0
            ...
            if self.validate_wyckoff and self.filter_invalid_wyckoff:
                if not self._validate_structure_wyckoff_consistency(wyckoff_indices, spacegroup_number, material_id):
                    log.warning(f"Structure {material_id} failed Wyckoff consistency check, filtering out")
    ...
    def _validate_entry(self, entry: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if len(entry["composition"]) != self.element_vocab_size:
                log.error(f"Composition vector has wrong size: {len(entry['composition'])} != {self.element_vocab_size}")
                return False
            ...
            if len(entry["lattice"]) != 6:
                log.error(f"Lattice parameters have wrong size: {len(entry['lattice'])} != 6")
                return False
            
            # Check that atom sequences have same length
            seq_len = len(entry["atom_types"])
            if len(entry["wyckoff"]) != seq_len:
                log.error(f"Wyckoff sequence length mismatch: {len(entry['wyckoff'])} != {seq_len}")
                return False
            
            if len(entry["atom_coords"]) != seq_len:
                log.error(f"Coordinates sequence length mismatch: {len(entry['atom_coords'])} != {seq_len}")
                return False
            
            # Validate coordinate dimensions
            for i, coords in enumerate(entry["atom_coords"]):
                if len(coords) != 3:
                    log.error(f"Coordinates at index {i} have wrong dimension: {len(coords)} != 3")
                    return False
    ...
    def create_dataset_from_csv(
        ...
        try:
            ...
            if cif_column not in df.columns:
                log.error(f"CIF column '{cif_column}' not found in CSV. Available columns: {list(df.columns)}")
                return
            ...
            structures = []
            for i, (_, row) in enumerate(tqdm(df.iterrows(), desc="Processing CIF strings", total=len(df))):
                try:
                    ...
                    try:
                        is_null = pd.isna(cif_string).item() if hasattr(pd.isna(cif_string), 'item') else bool(pd.isna(cif_string))
                    except (AttributeError, ValueError):
                        is_null = cif_string is None or (isinstance(cif_string, str) and cif_string.lower() in ['nan', 'null', ''])
                    ...
                    try:
                        current_material_id = material_ids[i] if i < len(material_ids) else f"row_{i}"
                    except:
                        current_material_id = f"row_{i}"
                    log.error(f"Error processing row {i} ({current_material_id}): {str(e)}")
                    self.error_count += 1
    ...
    def create_dataset_from_structure_list(
        ...
        dataset_entries = []
        for structure, material_id in tqdm(zip(structures, material_ids), desc="Processing structures"):
    ...
    def _save_dataset(self, structures: List[Dict[str, Any]], output_file: Path) -> None:
        ...
        for entry in structures:
            tensor_entry = {
                ...
                "spacegroup": entry["spacegroup"],  # Will be converted to tensor in dataset
                "lattice": torch.tensor(entry["lattice"], dtype=torch.float),
                "atom_types": entry["atom_types"],  # Will be converted to tensor in dataset
                "wyckoff": entry["wyckoff"],  # Will be converted to tensor in dataset
                "atom_coords": entry["atom_coords"],  # Will be converted to tensor in dataset
    ...
    def _validate_wyckoff_index(self, wyckoff_idx: int, spacegroup_num: int, material_id: str) -> tuple[bool, Optional[int]]:
        """
        ...
        try:
            ...
            if wyckoff_idx < 0 or wyckoff_idx >= self.wyckoff_vocab_size:
                log.warning(f"Wyckoff index {wyckoff_idx} out of bounds [0, {self.wyckoff_vocab_size}) for {material_id}")
                return False, 0  # Use padding index as fallback
            
            # Decode Wyckoff index to check space group consistency
            try:
                decoded_sg, decoded_letter = wyckoff_interface.index_to_wyckoff(wyckoff_idx)
                ...
                if decoded_sg != spacegroup_num:
                    ...
                    if self.strict_wyckoff_validation:
                        ...
                        if corrected_idx is not None:
                            log.info(f"Corrected Wyckoff index from {wyckoff_idx} to {corrected_idx} for {material_id}")
                ...
                log.error(f"Error decoding Wyckoff index {wyckoff_idx} for {material_id}: {e}")
                return False, 0
                
        except Exception as e:
            log.error(f"Error validating Wyckoff index {wyckoff_idx} for {material_id}: {e}")
            return False, 0
    ...
    def _find_valid_wyckoff_for_sg(self, spacegroup_num: int) -> Optional[int]:
        ...
        try:
            ...
            log.error(f"Error finding valid Wyckoff for space group {spacegroup_num}: {e}")
        return None
    
    def _validate_structure_wyckoff_consistency(self, wyckoff_indices: List[int], spacegroup_num: int, material_id: str) -> bool:
        ...
        if invalid_count / len(wyckoff_indices) > 0.5:
            log.warning(f"Structure {material_id} has {invalid_count}/{len(wyckoff_indices)} invalid Wyckoff indices")
            return False
    ...
    def _validate_entry_wyckoff_indices(self, entry: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            spacegroup_num = entry["spacegroup"]
            material_id = entry.get("material_id", "unknown")
            
            # Check each Wyckoff index
            for i, idx in enumerate(wyckoff_indices):
                if not isinstance(idx, int):
                    log.error(f"Wyckoff index at position {i} is not integer: {type(idx)}")
                    return False
                
                # Check bounds
                if idx < -2 or idx >= self.wyckoff_vocab_size:
                    log.error(f"Wyckoff index {idx} at position {i} out of bounds for {material_id}")
                    return False
    ...
    def _print_statistics(self) -> None:
        ...
        total = self.processed_count + self.error_count + self.skipped_count + self.wyckoff_filtered_count
        ...
        if total > 0:
            ...
            if self.wyckoff_filtered_count > 0:
                log.info(f"Wyckoff filter rate: {self.wyckoff_filtered_count/total*100:.1f}%")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Create dataset for Wyckoff encoder system")
    
    # Input options (mutually exclusive)
    input_group = parser.add_mutually_exclusive_group(required=True)
    input_group.add_argument("--input_dir", type=Path, help="Directory containing CIF files")
    ...
    # CSV options
    parser.add_argument("--cif_column", type=str, help="Name of column containing CIF strings")
    parser.add_argument("--id_column", type=str, help="Name of column containing material IDs (optional)")
    
    # Processing options
    parser.add_argument("--max_structures", type=int, help="Maximum number of structures to process")
    parser.add_argument("--element_vocab_size", type=int, default=100, help="Element vocabulary size")
    parser.add_argument("--cif_pattern", type=str, default="*.cif", help="Pattern for CIF files")
    
    # Wyckoff filtering options
    parser.add_argument("--no_wyckoff_filter", action="store_true", help="Disable Wyckoff index filtering")
    parser.add_argument("--lenient_wyckoff", action="store_true", help="Use lenient Wyckoff validation (allow space group mismatches)")
    parser.add_argument("--keep_invalid_wyckoff", action="store_true", help="Keep structures with invalid Wyckoff indices (correct them instead)")
    ...
    if args.input_dir:
        ...
        creator.create_dataset_from_csv(
            args.csv_file, args.cif_column, output_file, args.id_column, args.max_structures
        )
