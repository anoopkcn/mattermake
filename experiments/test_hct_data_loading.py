...
def test_dataset(data_dir: str, num_samples: int = 5):
    ...
    for idx in indices:
        ...
        atoms_match = torch.all(sample['atom_types'] == preload_sample['atom_types']).item()
...
def test_datamodule(data_dir: str):
    ...
    print(f"Test dataset size: {len(datamodule.test_dataset) if datamodule.test_dataset else 0}")
