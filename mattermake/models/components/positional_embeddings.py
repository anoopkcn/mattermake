...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000, base: int = 10000):
        super().__init__()
        self.dropout = nn.Dropout(p=dropout)
    ...
    def _apply_rotary_pos_emb(self, x: torch.Tensor, offset: int = 0) -> torch.Tensor:
        ...
        if seq_len + offset > self.max_len:
            raise ValueError(f"Sequence length ({seq_len + offset}) exceeds maximum length ({self.max_len})")
        
        # Get the right part of the precomputed frequency tensor
        cos = self.freqs_cos[offset:offset + seq_len].unsqueeze(0)  # [1, seq, d_model/2]
        sin = self.freqs_sin[offset:offset + seq_len].unsqueeze(0)  # [1, seq, d_model/2]
    ...
    def forward(self, x: torch.Tensor, offset: int = 0) -> torch.Tensor:
        ...
        is_seq_first = False
        if x.size(0) <= x.size(1) and x.size(0) <= 32:  # Heuristic to detect seq_len first
