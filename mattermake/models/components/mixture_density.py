...
class MixtureOfGaussiansHead(MixtureDensityHead):
    ...
    def get_distribution(self, params: Dict[str, torch.Tensor]) -> dist.Distribution:
        ...
        clean_logits = torch.nan_to_num(params["weights_logits"], nan=0.0, posinf=0.0, neginf=-1e5)
        ...
        if torch.isnan(weights_probs).any() or torch.isinf(weights_probs).any():
            ...
            print(f"Using uniform fallback for mixture weights with shape: {params['weights_logits'].shape}")
        ...
        for i in range(self.output_dim):
            try:
                ...
                loc = torch.zeros((params["means"].shape[0],), device=params["means"].device)
                scale = torch.ones((params["means"].shape[0],), device=params["means"].device)
        ...
        if len(distributions) > 0:
            ...
            means = torch.zeros((batch_size, output_dim), device=distributions[0].mean.device)
            stds = torch.ones((batch_size, output_dim), device=distributions[0].stddev.device)
...
class MixtureOfWrappedNormalsHead(MixtureDensityHead):
    ...
    def get_distribution(self, params: Dict[str, torch.Tensor]) -> dist.Distribution:
        ...
        for key, tensor in params.items():
            if torch.isnan(tensor).any() or torch.isinf(tensor).any():
                print(f"Warning: NaN/Inf detected in coordinate {key} with shape {tensor.shape}")
        ...
        if torch.isnan(weights_probs).any() or torch.isinf(weights_probs).any():
            # Use uniform distribution as fallback if still invalid
            batch_size, output_dim, n_mix = params["weights_logits"].shape
            print(f"Using uniform fallback for coordinate mixture weights with shape: {params['weights_logits'].shape}")
            weights_probs = torch.ones_like(params["weights_logits"]) / n_mix
            
        # Clean means and scales with more aggressive handling
        means = torch.nan_to_num(params["means"], nan=0.5) # Default to middle of [0,1) range
        scales = torch.nan_to_num(params["scales"], nan=0.1)
        # Ensure scales are positive and not too large for periodic values
        scales = torch.clamp(scales, min=1e-3, max=0.2) # Limit max scale to 0.2 for wrapped values
        ...
        for i in range(self.output_dim):
            try:
                ...
                print(f"Warning: Error creating wrapped distribution for dimension {i}: {e}")
                loc = 0.5 * torch.ones((params["means"].shape[0],), device=params["means"].device)
                scale = 0.1 * torch.ones((params["means"].shape[0],), device=params["means"].device)
                distributions.append(dist.Normal(loc, scale))
