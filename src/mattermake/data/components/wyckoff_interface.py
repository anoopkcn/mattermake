...
class WyckoffInterface:
    ...
    def _create_global_mapping(self):
        ...
        try:
            for sg_key, sg_data in self.db.data.items():
                ...
                if not hasattr(sg_data, 'wyckoff_positions') or not sg_data.wyckoff_positions:
                ...
                for wyckoff_pos in sg_data.wyckoff_positions:
                    if not hasattr(wyckoff_pos, 'letter'):
                        log.warning(f"Wyckoff position missing letter in space group {sg_key}")
                        continue
    ...
    def wyckoff_to_index(self, sg_num: int, letter: str) -> int:
        ...
        if index == 0 and key != (0, 'pad'):
            log.debug(f"Wyckoff position ({sg_num}, '{letter}') not found in mapping, using padding index")
        return index
    ...
    def get_valid_wyckoff_letters(self, sg_num: int) -> List[str]:
        ...
        if hasattr(sg_data, 'wyckoff_positions'):
            ...
            if hasattr(wyckoff_positions, '__iter__'):
                return [wp['letter'] for wp in wyckoff_positions if isinstance(wp, dict) and 'letter' in wp]
            else:
    ...
    def create_sg_mask(self, sg_numbers: torch.Tensor, device: torch.device) -> torch.Tensor:
        ...
        mask = torch.zeros(batch_size, effective_vocab_size, dtype=torch.bool, device=device)
