...
class CrystalConstraintHandler:
    ...
    def update_wyckoff_position(self, batch_idx: int, token_id: int) -> None:
        ...
        if token_id >= 3000:
            ...
            if multiplicity <= 0:
                token_name = self.token_maps.get("token_to_wyckoff_mult", {}).get(str(token_id))
    ...
    def get_wyckoff_mask(self, batch_idx: int, use_combined_tokens: bool = True) -> Optional[torch.Tensor]:
        ...
        if not use_combined_tokens:
            ...
            allowed_wyckoff = self.sg_wyckoff_mapping.get_allowed_wyckoff_positions(space_group)
            
            # Create a mapping from token_id to validity
            token_mask = {}
            
            # For each token in the vocabulary
            for token_key, token_id in self.token_maps.get("wyckoff_mult_to_token", {}).items():
            ...
            max_token_id = max(token_mask.keys()) if token_mask else 3000 + (230 * 100) + 26
